...
public class Validation {
    ...
    protected static final Pattern EMAIL_PATTERN = Pattern.compile("^(?=.{1,64}@)[A-Za-z0-9_\\+-]+(\\.[A-Za-z0-9_-]+)*@[A-Za-z0-9][A-Za-z0-9-]*(\\.[A-Za-z0-9-]+)*(\\.[A-Za-z]{2,})$");
    ...
    /**
     * Check to see if a VALUE appears to be a disallowed URL.
     *
     * No longer presumes to add protocol; it must be provided to be a proper
     * URL value.
     *
     * @param value the VALUE to check
     * @return true if appears to be a disallowed URL, false otherwise
     */
    public static boolean isDisallowedUrl(String value) {
        return ( null==value ) ?
                false :
                DISALLOWED_URL_PATTERN.matcher(value.trim()).matches();
    }
